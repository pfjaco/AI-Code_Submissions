...
class AStarNode(Node):
    ...
    def heuristic_function(self):
        z = []
        for coord_in in self.END_COORDS:
            x = self.coord[0] - coord_in[0]
            y = self.coord[1] - coord_in[1]
            z_d = np.sqrt((x**2) + (y**2))
            z.append(z_d)    
        return min(z)
...
class Agent:
    ...
    def goal_test(self, node):
        if node.coord in self.maze.end:
            return True
        else: return False
    ...
    def bfs(self):
        ...
        reached.append(start_node.coord)
        frontier = Queue()
        frontier.put(start_node)
        while (frontier.empty() == False):
            curr_node = frontier.get()
            for child in self.expand_node(curr_node):
                if self.goal_test(child):
                    return (True, child.trace_back(), self.expansion_history)
                else:
                    if((child.coord in reached) == False):
                        reached.append(child.coord)
                        frontier.put(child)
    ...
    def ucs(self):
        ...
        while (frontier.empty() == False):
            curr_node = frontier.get()
            if self.goal_test(curr_node):
                return (True,curr_node.trace_back(),self.expansion_history)
            for child in self.expand_node(curr_node):
                if((child.coord in reached) == False):
                    reached.append(child.coord)
                    frontier.put(child)
    ...
    def astar(self):
        ...
        reached = []
        start_node = AStarNode(self.maze.start)
        reached.append(start_node.coord)
        frontier = PriorityQueue()
        frontier.put(start_node)
        while (frontier.empty() == False):
            ...
            if self.goal_test(curr_node):
                return (True,curr_node.trace_back(),self.expansion_history)
            #print(curr_node.coord,curr_node.h_val, curr_node.path_cost)
            for child in self.expand_node(curr_node):
                if((child.coord in reached) == False):
                    reached.append(child.coord)
                    frontier.put(child)
